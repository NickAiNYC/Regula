...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "subscription_tier",
            postgresql.ENUM(
                "trial", "basic", "professional", "enterprise", name="subscription_tier"
            ),
            nullable=False,
    ...
    op.create_table(
        ...
        sa.Column(
            "cola_percentage", sa.Numeric(5, 4), nullable=False, server_default="0.0284"
        ),
        sa.Column("is_active", sa.Boolean(), nullable=False, server_default="true"),
        sa.Column(
            "created_at",
    ...
    op.create_index(
        "idx_claims_org_date",
        "claims",
        ["organization_id", "service_date"],
        postgresql_using="btree",
    )
    op.create_index(
        "idx_claims_payer_date",
        "claims",
    ...
    op.create_index(
        "idx_claims_violation",
        "claims",
        ["is_violation", "service_date"],
        postgresql_using="btree",
    )
    ...
    # Rate database
    op.create_index(
        "idx_rates_cpt_date", "rate_database", ["cpt_code", "effective_date"]
    )
