...
class RecoveryTracker:
    ...
    async def record_recovery(
        ...
        recovery = {
            ...
            "recovery_rate": float((recovered_amount / original_underpayment) * 100) if original_underpayment > 0 else 0,
            "payer": payer,
    ...
    async def get_recovery_metrics(
        ...
        if start_date:
            filtered_recoveries = [
                r for r in filtered_recoveries
        ...
        # Calculate metrics
        total_underpaid = sum(Decimal(str(r["original_underpayment"])) for r in filtered_recoveries)
        total_recovered = sum(Decimal(str(r["recovered_amount"])) for r in filtered_recoveries)
        
        recovery_yield = (total_recovered / total_underpaid * 100) if total_underpaid > 0 else 0
        
        avg_recovery_amount = total_recovered / len(filtered_recoveries)
        avg_recovery_rate = sum(r["recovery_rate"] for r in filtered_recoveries) / len(filtered_recoveries)
        
        # Payer breakdown
        payer_stats = defaultdict(lambda: {"count": 0, "recovered": Decimal("0"), "underpaid": Decimal("0")})
        ...
        payer_breakdown = {
            payer: {
                ...
                "yield": float((stats["recovered"] / stats["underpaid"] * 100) if stats["underpaid"] > 0 else 0)
            }
    ...
    async def calculate_roi(
        ...
        return {
            ...
            "avg_recovery_per_dollar_spent": float(total_recovered / total_costs) if total_costs > 0 else 0,
    ...
    async def get_pending_appeals_summary(self) -> Dict:
        ...
        total_outstanding = sum(
            Decimal(str(a.get("expected_recovery", 0)))
            for a in self.pending_appeals
        )
        ...
        for appeal in self.pending_appeals:
            ...
            by_payer[payer]["amount"] += Decimal(str(appeal.get("expected_recovery", 0)))
    ...
    async def generate_recovery_report(
        ...
        roi = await self.calculate_roi(provider_id, platform_costs, start_date, end_date)
