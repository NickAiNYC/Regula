...
class AetnaCommercialAdapter(BasePayerAdapter):
    ...
    async def detect_underpayment(
        ...
        **kwargs,
        ...
        return {
            ...
            "delta": delta,
            "reason": (
                "Payment below contracted allowed amount" if is_violation else None
            ),
    ...
    async def apply_edits(
        ...
        **kwargs,
        ...
        if diagnosis_codes and not self._supports_medical_necessity(
            cpt_code, diagnosis_codes
        ):
            violations.append(
                {
    ...
    async def _estimate_medicare_rate(
    ...
    def _supports_medical_necessity(
        self, cpt_code: str, diagnosis_codes: List[str]
    ) -> bool:
