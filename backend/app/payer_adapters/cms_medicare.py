...
class CMSMedicareAdapter(BasePayerAdapter):
    ...
    async def apply_edits(
        ...
        **kwargs,
        ...
        ncci_result = await self._check_ncci_edits(
            cpt_code, kwargs.get("other_cpt_codes", [])
        )
    ...
    async def _get_rvus(self, cpt_code: str, service_date: date) -> Optional[Dict]:
        ...
        rvus_database = {
            "90791": {
                "work_rvu": Decimal("2.96"),
                "pe_rvu": Decimal("1.47"),
                "mp_rvu": Decimal("0.25"),
            },
            "90832": {
                "work_rvu": Decimal("1.46"),
                "pe_rvu": Decimal("0.72"),
                "mp_rvu": Decimal("0.12"),
            },
            "90834": {
                "work_rvu": Decimal("2.19"),
                "pe_rvu": Decimal("1.09"),
                "mp_rvu": Decimal("0.18"),
            },
            "90837": {
    ...
    async def _get_gpci(self, locality_code: str, service_date: date) -> Dict:
        ...
        gpci_database = {
            "00": {
                "work_gpci": Decimal("1.094"),
                "pe_gpci": Decimal("1.264"),
                "mp_gpci": Decimal("0.879"),
            },
            "01": {
                "work_gpci": Decimal("1.011"),
                "pe_gpci": Decimal("1.088"),
                "mp_gpci": Decimal("0.652"),
            },
        }
        return gpci_database.get(
            locality_code,
            {
    ...
    def _apply_modifier_adjustments(
        self, base_amount: Decimal, modifiers: List[str]
    ) -> Decimal:
    ...
    async def _check_lcd(
        self, cpt_code: str, diagnosis_codes: List[str], service_date: date
    ) -> Dict:
