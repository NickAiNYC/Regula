...
class AppealSuccessOptimizer:
    ...
    async def analyze_appeal_opportunity(
        ...
        expected_recovery = underpayment_amount * (
            Decimal(str(success_probability)) / Decimal("100")
        )
    ...
    async def prioritize_appeals(self, pending_violations: List[Dict]) -> List[Dict]:
        """
        ...
        for violation in pending_violations:
            analysis = await self.analyze_appeal_opportunity(
                claim_data=violation.get("claim", {}), violation_data=violation
            )
    ...
    async def _calculate_success_probability(
        self, claim_data: Dict, violation_data: Dict, provider_context: Optional[Dict]
    ) -> float:
        """
    ...
    def _estimate_appeal_cost(
        self, claim_data: Dict, success_probability: float
    ) -> Decimal:
        """
    ...
    def _should_pursue_appeal(
        self, success_probability: float, roi_ratio: float, underpayment_amount: Decimal
    ) -> bool:
        """
    ...
    def _calculate_priority_score(
        self, success_probability: float, underpayment_amount: Decimal, roi_ratio: float
    ) -> int:
        """
    ...
    def _recommend_strategy(
        self, success_probability: float, claim_data: Dict, violation_data: Dict
    ) -> str:
        """
    ...
    def _get_required_documents(
        self, claim_data: Dict, violation_data: Dict
    ) -> List[str]:
        """
    ...
    def _generate_appeal_narrative(
        self, claim_data: Dict, violation_data: Dict, strategy: str
    ) -> str:
        """
