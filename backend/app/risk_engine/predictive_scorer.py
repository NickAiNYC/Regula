...
class PredictiveUnderpaymentScorer:
    ...
    async def score_claim(
        ...
        recommendation = self._generate_recommendation(risk_score, risk_category, claim_data)
    ...
    def _extract_features(
        ...
        if historical_context:
            features["provider_violation_rate"] = historical_context.get("violation_rate", 0.0)
            features["provider_avg_delta"] = historical_context.get("avg_underpayment", 0.0)
        ...
        if historical_context:
            ...
            features["payer_avg_payment_ratio"] = payer_stats.get("avg_payment_ratio", 1.0)
        
        # Interaction features
        features["amount_ratio"] = (
            features["billed_amount"] / features["expected_amount"]
            if features["expected_amount"] > 0 else 1.0
        )
    ...
    def _get_feature_names(self) -> List[str]:
        ...
        return [
            "cpt_code", "payer", "billed_amount", "expected_amount",
            "day_of_week", "month", "quarter", "is_fiscal_year_end",
            "geo_region", "provider_violation_rate", "provider_avg_delta",
            "provider_claim_volume", "payer_violation_rate",
            "payer_avg_payment_ratio", "amount_ratio"
        ]
