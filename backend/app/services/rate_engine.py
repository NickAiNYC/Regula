...
class RateEngine:
    ...
    async def get_mandate_rate(
        self, cpt_code: str, service_date: date, geo_region: str = "upstate"
        ...
        if self.cache:
            ...
            await self.cache.setex(cache_key, settings.CACHE_TTL, cache_value)
    ...
    async def bulk_check_violations(self, claims_data: list[Dict]) -> list[Dict]:
        """
        ...
        for claim in claims_data:
            ...
            results.append({**claim, **violation_info})

        return results


async def get_rate_engine(
    db: AsyncSession, cache: Optional[Redis] = None
) -> RateEngine:
