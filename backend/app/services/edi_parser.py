...
class EDI835Parser:
    ...
    def _parse_clp_segment(self, elements: List[str]) -> Dict:
        ...
        try:
            return {
                ...
                "billed_amount": self._parse_decimal(elements[3]) if len(elements) > 3 else None,
                "claim_paid_amount": self._parse_decimal(elements[4]) if len(elements) > 4 else None,
    ...
    def _parse_svc_segment(self, elements: List[str]) -> Optional[Dict]:
        ...
        try:
            ...
            return {
                "cpt_code": cpt_code,
                "billed_amount": self._parse_decimal(elements[2]) if len(elements) > 2 else None,
                "paid_amount": self._parse_decimal(elements[3]) if len(elements) > 3 else Decimal("0.00"),
                "units": int(elements[4]) if len(elements) > 4 and elements[4] else 1,
                "dos": None  # Will be filled by DTM segment
            }
        except Exception as e:
            logger.warning("failed_to_parse_svc_segment", error=str(e), elements=elements)
            return None
    ...
    def _parse_dtm_segment(self, elements: List[str]) -> Optional[Dict]:
        ...
        try:
            ...
            logger.warning("failed_to_parse_dtm_segment", error=str(e), elements=elements)
        return None
