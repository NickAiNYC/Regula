...
class DocumentGenerator:
    ...
    async def generate_appeal_package(
        self, claim: Claim, provider: Provider, organization: Organization
    ) -> bytes:
        """
    ...
    async def batch_generate_demand_letters(
        self, violations_by_payer: dict, provider: Provider, organization: Organization
    ) -> dict:
        """
        ...
        for payer_name, violations in violations_by_payer.items():
            try:
                ...
                logger.error("batch_generation_error", payer=payer_name, error=str(e))
