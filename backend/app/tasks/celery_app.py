...
celery_app = Celery(
    'regula',
    broker=str(settings.CELERY_BROKER_URL) if settings.CELERY_BROKER_URL else 'redis://localhost:6379/0',
    backend=str(settings.CELERY_RESULT_BACKEND) if settings.CELERY_RESULT_BACKEND else 'redis://localhost:6379/1',
