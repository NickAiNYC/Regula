...
from app.models import Claim, Provider
...
def process_edi_file(
    self, file_content: str, provider_id: str, organization_id: str, file_name: str
) -> dict:
    """
        ...
        try:
            ...
            self.update_state(
                ...
                meta={
                    "current": 25,
                    "total": 100,
                    "status": f"Parsed {len(parsed_claims)} claims",
                },
            )
            ...
            async with AsyncSessionLocal() as db:
                ...
                for idx, claim_data in enumerate(parsed_claims):
                    ...
                    if violation_info["mandate_rate"] is not None:
                        claim = Claim(
                            ...
                            geo_adjustment_factor=violation_info[
                                "geo_adjustment_factor"
                            ],
                    ...
                    if len(claims_to_insert) >= batch_size:
                        ...
                        self.update_state(
                            ...
                            meta={
                                "current": progress,
                                "total": 100,
                                "status": f"Processed {idx}/{len(parsed_claims)} claims",
                            },
                        )
                ...
                logger.info("edi_processing_complete", **result)

                return result

        except Exception as e:
            logger.error("edi_processing_error", file_name=file_name, error=str(e))

            # Retry on transient errors
            if "timeout" in str(e).lower() or "connection" in str(e).lower():
                raise self.retry(exc=e)

            return {"status": "error", "message": str(e), "claims_processed": 0}
