...
class Claim(Base, TimestampMixin):
    ...
    geo_adjustment_factor = Column(
        Numeric(5, 3)
    )  # 1.065 for NYC, 1.025 for LI, 1.0 for upstate
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "delta = mandate_rate - paid_amount", name="check_delta_calculation"
        ),
    )
...
class RateDatabase(Base, TimestampMixin):
    ...
    __table_args__ = (Index("idx_rate_cpt", "cpt_code"),)
