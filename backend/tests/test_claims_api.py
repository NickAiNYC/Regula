...
async def test_claim_upload_success(
    client: AsyncClient, auth_headers: dict, test_provider: Provider, test_rates
):
...
SE*5*0001~"""
    ...
    response = await client.post(
        "/api/v1/claims/upload", files=files, headers=auth_headers
    )
...
async def test_list_claims(
    client: AsyncClient, auth_headers: dict, test_provider: Provider
):
    """Test listing claims with pagination"""
    response = await client.get(
        "/api/v1/claims", headers=auth_headers, params={"page": 1, "per_page": 10}
    )
...
@pytest.mark.asyncio
async def test_list_claims_with_filters(client: AsyncClient, auth_headers: dict):
    """Test claims listing with filters"""
    response = await client.get(
        "/api/v1/claims",
        headers=auth_headers,
        params={"payer": "Aetna", "is_violation": True, "page": 1, "per_page": 50},
    )
...
async def test_get_claim_by_id(
    client: AsyncClient, auth_headers: dict, db_session, test_provider: Provider
):
    ...
    response = await client.get(f"/api/v1/claims/{claim.id}", headers=auth_headers)
...
@pytest.mark.asyncio
async def test_upload_malformed_edi(client: AsyncClient, auth_headers: dict):
